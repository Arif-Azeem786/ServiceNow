...
////////////////////FIBROID DETECTION SOURCE CODE VERSION 2.0  (READY FOR DEVELOPMENT)
//CODE CLEANED LAST ON : 27-02-2025//
//CODE LENGTH OF FILE FIBROIDDETECTION.JSX LINE 16 - LINE 173//
//NUMBER OF SANITY CHECKS DONE DURING DEVELOPMENT : 6 CHECKS//
// DATE OF DEVELOPMENT START OF FIBROIDDETECTION.JSX 26/2/2025 - 27/2/2025
///////////////////////// BASIC INFORMATION ABOUT THE FILE //////////////////////////////////////////
// This React component is a key interface for the Fibroid Detection application.
// It allows users to upload a video, analyze it for fibroid detection, and generate a detailed report.
...
The video is processed to extract frames, which are then analyzed for fibroid detection.
...
//Data Flow:
/*
User uploads video > Video is processed to extract frames > Frames are analyzed for fibroid detection > Analysis results are displayed > User generates a detailed report.
*/
...
import FramesPreview from "./Fibroidframes";
...
export default function FibroidDetection({ logoutFunction }) {
  ...
  const sendFramesToBackend = async (frames) => {
    try {
      const response = await fetch("https://servicenow-1-rcpd.onrender.com/ai/fibroid-detection/", {
        method: "POST",
        headers: {
      ...
      while (true) {
            ...
            try {
              ...
              if (parsedData.success) {
                ...
                if (
                  ...
                  if (frameData.class_name === "fibroid") {
                    const isDuplicate = fibroidframes.some(
                      (frame) => frame.num_fibroid === frameData.num_fibroid
                    );

                    if (!isDuplicate) {
                      fibroidframes.push(frameData);
        ...
        dataBuffer = parts[parts.length - 1]; // Preserve incomplete message
      }
  ...
  useEffect(() => {
    const sendFinalFramesToBackend = async () => {
      ...
      try {
        const response = await fetch(
          "https://servicenow-1-rcpd.onrender.com/ai/generate-findings1/",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
