...
class DeploymentConfig:
    ...
    NO_FIBROID_FRAME_DIR = "frames/no_fibroid_frames"
...
clear_directory(DeploymentConfig.FIBROID_FRAME_DIR)
clear_directory(DeploymentConfig.NO_FIBROID_FRAME_DIR)
...
def analyse_result(result, frame_number, frame_with_results, num_fibroid, confidence, class_name,boxes):
    ...
    if classes_detected.size > 0:
        ...
        output_path = os.path.join(DeploymentConfig.FIBROID_FRAME_DIR, f"frame_{frame_number:04d}.jpg")
        cv2.imwrite(output_path, frame_with_results)
    else:
        output_path = os.path.join(DeploymentConfig.NO_FIBROID_FRAME_DIR, f"frame_{frame_number:04d}.jpg")
        cv2.imwrite(output_path, frame_with_results)
        
    return num_fibroid, class_name, confidence, boxes

def process_frames1(frames):
    """
    Process a list of frames, perform AI predictions, analyze the results, 
    and yield results for each frame in real-time.
    ...
    for frame_number, frame_data in enumerate(frames):
        try:
            ...
            for result in results_frame:
                ...
                num_fibroid, class_name, confidence, boxes = analyse_result(
                    result, frame_number, frame_with_results, num_fibroid, confidence, class_name, boxes
                )
...
def process_video(video):
    ...
    global fibroid_frames
    global fibroid_confidence
    frame_number = 0
    num_fibroid = 0
    fibroid_frames = []
    no_Detection_frames = []
    ...
    while cap.isOpened():
        ...
        for result in results:
            ...
            boxes = np.zeros((1,4))
            num_fibroid, class_name, confidence, boxes = analyse_result(result, frame_number, frame_with_index, num_fibroid, confidence, class_name, boxes)
        ...
        if class_name == 'fibroid':
            fibroid_frames.append(frame_with_index_rgb)
            fibroid_confidence.append((confidence,frame_number,boxes))
        if class_name == 'No_detection':
            no_Detection_frames.append(frame_with_index_rgb)
    ...
    if len(fibroid_confidence)>0:
        max1 = max(fibroid_confidence)
        fibroid_confidence.remove(max1)
        max2 = max(fibroid_confidence)
        frame_number_1 = max1[1]
        box1 = max1[2]
        frame_number_2 = max2[1]
        box2 = max2[2]
        img1 = os.path.join(DeploymentConfig.FIBROID_FRAME_DIR, f"frame_{frame_number_1:04d}.jpg")
        img2 = os.path.join(DeploymentConfig.FIBROID_FRAME_DIR, f"frame_{frame_number_2:04d}.jpg")
